...
#
#
#    Protect those interfaces from the pesky red team!!!
#    Lots of AI, but a TON of time went into the script. A ton had to be figured out and worked out by hand.
#    I thought my Splunk forwarder script was a chore to make, but man... this takes the cake    
#
...
# Function to fix Debian repository sources
fix_debian_sources() {
    if [ -f /etc/debian_version ]; then
        log_message "Detected Debian system"
        if ! command -v lsb_release > /dev/null; then
            log_message "lsb_release not found, attempting to install..."
            apt-get update && apt-get install -y lsb-release || {
                log_message "Failed to install lsb-release. Using fallback method."
                codename=$(cat /etc/os-release | grep VERSION_CODENAME | cut -d'=' -f2)
                if [ -z "$codename" ]; then
                    echo "Cannot determine Debian codename. Please install lsb-release or set sources manually."
                    exit 1
                fi
            }
        fi
        ...
        if [ -z "$codename" ]; then
            log_message "Failed to determine Debian codename. Please check your system configuration."
            echo "Failed to determine Debian codename. Please check your system configuration."
            exit 1
        fi
        cp /etc/apt/sources.list /etc/apt/sources.list.bak
        log_message "Backed up /etc/apt/sources.list to /etc/apt/sources.list.bak"
        for repo in "deb http://deb.debian.org/debian $codename main" \
                    "deb http://deb.debian.org/debian $codename-updates main" \
                    "deb http://security.debian.org/debian-security $codename/updates main"; do
            if ! grep -Fxq "$repo" /etc/apt/sources.list; then
                echo "$repo" >> /etc/apt/sources.list
                log_message "Added $repo to /etc/apt/sources.list"
            fi
        done
        apt-get update || {
            log_message "Failed to update package lists. Please check your network connection or sources.list."
            echo "Failed to update package lists. Please check your network connection or sources.list."
            exit 1
        }
    fi
}
...
if ! command -v inotifywait > /dev/null; then
    ...
    if command -v apt-get > /dev/null; then
        ...
        if [ -f /etc/redhat-release ]; then
            ...
            if echo "$release" | grep -q "CentOS Linux release 7"; then
                log_message "Detected CentOS 7"
                if ! yum repolist | grep -q epel; then
                    log_message "EPEL repository not enabled, installing epel-release..."
                    yum install -y epel-release || {
                        log_message "Failed to install epel-release. Please install manually."
                        echo "Failed to install epel-release. Please run: sudo yum install epel-release"
                        exit 1
                    }
                fi
            fi
...
backup_config() {
    ...
    local CONFIG_PATH=$(get_config_path "$manager")
    local IS_DIR=$(get_is_dir "$manager")
    local MANAGER_BACKUP_DIR="$BACKUP_DIR/$manager"#!/bin/bash
